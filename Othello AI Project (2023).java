...
public class Reversi
{
	...
	//**********************************************************************
	{
		...
		System.out.println("\t     Author: The Incredible Alexis Evans\n\n"); // <-- Insert Your Name Here!
    ...
    public static int canPutPiece(int X, int Y, int myColor)
	//**********************************************************************
	//** putPiece
	//**     Now that we know there's a valid move, we have to flip the
	//**     pieces around. Following the pattern of checkMove, we scan in
	...
	{
        int total = 0;
		total += fakeflipPiecesThisDir(X, Y, -1, -1, myColor);
		total += fakeflipPiecesThisDir(X, Y, -1,  0, myColor);
		total += fakeflipPiecesThisDir(X, Y, -1,  1, myColor);
		total += fakeflipPiecesThisDir(X, Y,  0, -1, myColor);
		total += fakeflipPiecesThisDir(X, Y,  0,  1, myColor);
		total += fakeflipPiecesThisDir(X, Y,  1, -1, myColor);
		total += fakeflipPiecesThisDir(X, Y,  1,  0, myColor);
		total += fakeflipPiecesThisDir(X, Y,  1,  1, myColor);
	...
	public static int fakeflipPiecesThisDir(int X, int Y, int deltaX, int deltaY, int myColor)
	//**********************************************************************
	//** flipPiecesThisDir
	//**     Search a given square in a given direction, and flip pieces
	//**     around if appropriate. Using logic similar to checkMove, if
	...
	//**********************************************************************
	{
		...
		currX = X + deltaX;
		currY = Y + deltaY;
		if (currX == -1 || currX == 8 || currY == -1 || currY == 8) return count;
		if (board[currY][currX] != enemyColor) return count;
		...
		if (board[currY][currX] != myColor) return count;
	...
	//**********************************************************************
	{
       ...
       int[][] howMany = new int[8][8];
       String yay = "ABCDEFGH";
        
       if (!moreMoves(playerColor)) {
           return;
       }
       for (int i = 0; i < 8; i++) {
           for (int j = 0; j < 8; j++) {
               validMove[i][j] = checkMove(i,j,playerColor);
           }
       }
       int r = 0;
       int c = 0;
       ...
       for (int i = 0; i < 8; i+= 7) {
           for (int j = 0; j < 8; j+=7) {
               if (validMove[i][j]) {
                   putPiece(i,j,playerColor);
                   r = i; c = j;
                   System.out.println("Computer placed a piece at: " + yay.charAt(r) + (c+1));
                   return;
    ...
    //biggest piece flipper
    
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                if (validMove[row][col]) {
                    howMany[row][col] = canPutPiece(row,col,playerColor);
        ...
        int biggest = howMany[0][0];

        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                if (howMany[row][col] > biggest) {
                    biggest = howMany[row][col];
                    r = row; c = col;
